...
**`schema/guide-v1.json` is a real file now**, so `$schema` in an emitted guide points at something. The JSON Schema had been living as a fenced block inside `guide-schema.md` §4; §4 is now a pointer, because two copies of one format is one copy too many. The `$id` is `https://raw.githubusercontent.com/artalar/tabthrough/main/schema/guide-v1.json`, which resolves today.
