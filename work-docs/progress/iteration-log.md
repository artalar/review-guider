...
**Track D and E.** README is rewritten for the marketplace: how it works, the three entry points, the key table, the safety model in five bullets, `.guide.json` in one example, and a ten-row known-limitations table drawn from the P2 edge budget plus the real gaps. Track E has a draft at `.agents/skills/guide-reviewer/SKILL.md` (mirrored into `.cursor/skills/`) — the write path for an agent emitting a sidecar, normative against guide-schema.md, including the prohibitions, since "no quizzes, no scores" is a product principle an eager model will otherwise violate helpfully. The JSON Schema is still only inside guide-schema.md §4 and is not served at its `$id`, so `$schema` in an emitted document currently resolves to nothing.
