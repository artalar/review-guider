...
#### Review 008 fix items — gate on P0-N3 approval

Filed by the Reviewer from [review 008](./reviews/008.md). **F1 and F2 block any user running
rebase mode.** P0-N4 does not depend on this code and may proceed in parallel. Severity letters
are the review's own; B1 / B2 / M1 / M2 were verified by execution.

| ID | Fix | Review ref | Where | Blocks | Status |
|----|-----|-----------|-------|--------|--------|
| **P0-N3-F1** | Finish must not act on a session that ended while the untracked pick was open: re-check `active` + same `session` + `rebaseOwnership` after every await, re-read `readOwnership().ours` before running, `try/finally` on `pauseOwnership` | **B1 + m1** | `src/model/session.ts` `finishSession` | dogfood, J5 | **Fixed** |
| **P0-N3-F2** | Applicability refuses a merge commit as `after` (`after^2` exists) and merges in `base..HEAD`; after `startRebase` returns 0, verify ownership before `active` — abort if stopped elsewhere, never reset if git ran through (name `ORIG_HEAD`) | **B2** | `src/git/rebase.ts`, `src/model/session.ts` `startPreview` / `startSession` | dogfood, J5 | **Fixed** |
| **P0-N3-F3** | Forward `rebase --continue` stdout / stderr on the ok path ("Applying autostash resulted in conflicts" exits 0) | **M1** | `src/model/session.ts` `finishSession` | J5 | **Fixed** |
| **P0-N3-F4** | "Will run:" derived from `planIsolation` + `peekSidecarFinish` — range base, guide `defaults.finish`, root commit | **M2** | `src/model/session.ts` `startPreview` | D6 | **Fixed** |
| **P0-N3-F5** | `saveDocuments` before Finish, same refusal as Start | **M3** | `src/model/session.ts` `finishSession` | J5 | **Fixed** |
| **P0-N3-F6** | No abort signal on `startRebase`; if cancelled while it ran, `rebase --abort` after it returns. **Re-review:** record `rebaseOwnership` synchronously when `startRebase` returns 0 (before `readOwnership`), or hold the no-`abort()` guard until it is set — a Cancel in that window orphans the rebase silently (PROBE-A) — plus T9 | **M4** | `src/model/session.ts` `startSession` | dogfood | **Fixed** |
| **P0-N3-F7** | Forward `--abort` results through `reportGit`; distinct notice for "stopped on a replay conflict" instead of "Review finished." | **m2 + m3** | `src/model/session.ts` | UX | **Fixed** |
| **P0-N3-F8** | Tests: Finish/Cancel race (T1), merge-as-after (T2), model-level Finish / Cancel / deactivate with recording `exec` (T3), autostash pop conflict (T4), Will-run fidelity (T5), spawn the shipped `dist/sequence-editor.cjs` (T6), staged WIP round trip (T7) | **T1–T7** | `test/integration/rebase-driver.test.ts`, new model rows | P0-N3 gate | **Fixed** |
| **P0-N3-F9** | `GIT_EDITOR=true` on Start; union dirty count + "staged comes back unstaged" note; Retry scoped to the amend; shell-escape `$` `` ` `` `\` in the editor path; move the `.ts` strip-types hack to the harness | **m4–m7** | `src/git/rebase.ts`, `src/model/session.ts` | polish | **Fixed** |
| **P0-N3-F10** | Cancel must not run while Finish's writes are in flight: `finishing` (with a legal `finishing → active` for declined Retry / plain failure) or an in-flight flag `cancelSession` awaits; re-check `finishSessionStillOpen` after every `runFinishRebase` before teardown (PROBE-C: `--abort -> 0` alongside `commit --amend -> 128 cannot lock ref 'HEAD'`) — plus T10. **Second re-review:** the widened `finishSessionStillOpen` (`active \|\| finishing`) also guards the five **pre-write** checkpoints, so a Cancel whose `--abort` is mid-flight (teardown claims `finishing` synchronously) no longer stops Finish — P6: `add -u` / `commit --amend` / `--continue` completed, then `--abort -> 128`, toasts "Review finished." + "fatal: no rebase in progress" + "Review cancelled.", HEAD rewritten on a Cancel. Pre-write checks must require `active`; only post-write checks accept `finishing` — plus T11 | **M5** (re-review) | `src/model/session.ts` `finishSession` / `cancelSession`, `src/model/sidebar.ts` | dogfood, J5 | **Confirmed closed** (third re-review, P6 with a 1 s `--abort`: no writes, one abort, HEAD intact) |
| **P0-N3-F11** | Clean Finish says "Review finished." — forward `--continue` output only when conflicts / autostash remain, strip `\r` progress (m8); on `!ours` in Finish tear down with `reason: 'ownership'` and run `syncRebaseOwnership()` after unpausing (m9); optional `--no-autosquash` on Start (m10) | **m8–m10** (re-review) | `src/model/session.ts`, `src/git/rebase.ts` | UX, D5 | **Fixed** |
| **P0-N3-F12** | Cancel in `starting` never waits: `take(startSettled, value => value === attempt)` is a **mapper**, its first evaluation is synchronous, and it returns the plain value `false` — `await wrap(false)` resolves at once (verified against `@reatom/core` 1001.3.0 standalone and in P1: `cancelSession` resolved while `.git/rebase-merge/` still held the stopped rebase; `--abort` landed ~90 ms later). `deactivate()`'s `Promise.race([cancelSession('deactivate'), sleep(4000)])` therefore disposes before `startFailed` runs `--abort` — reload / close during a rebase Start can orphan the stopped rebase with WIP in its autostash. Use `take(startSettled, value => value === attempt \|\| throwAbort())` or an `atom<number \| null>` — plus T12 (park `guideSource.build`, `await cancelSession`, assert `--abort` already logged before awaiting `starting`) | **M6** (second re-review) | `src/model/session.ts` `cancelSession` | dogfood, T8 reload drill | **Confirmed closed** (third re-review, P1 for `cancel` and `deactivate`: `--abort` logged and `rebase-merge/` gone before Cancel returns) |
| **P0-N3-F13** | (m11) cancelled-start branch aborts on `state.rebase !== null` — with `started.code !== 0` because a rebase was already in progress it aborts the **user's** rebase (P3, verified): abort only when `started.code === 0`; refuse rebase mode in `assessRebase` / `startSession` while `gitState.rebase !== null \|\| operation !== null` with a hint. (m12) `tabthrough.cancel` `enablement` / `view/title` `when` are `sessionOpen`, so Cancel during `finishing` is a silent no-op from the palette / title (P4): notify or add a `sessionFinishing` context key. Lows: REFERENCE.md §take "resolves only when it returns truthy" is wrong for 1001.3.0; ADR 0005 state diagram lacks `finishing → active`; "Closing review…" spinner while the Retry notice is open | **m11 + m12 + Lows** (second re-review) | `src/model/session.ts` `startSession`, `package.json`, `.cursor/skills/reatom/REFERENCE.md`, ADR 0005 | D5, UX | **Confirmed closed** (third re-review, P3: foreign rebase started mid-`rebase -i`, Cancel landed, no `--abort`; REFERENCE.md + spinner copy carried as Lows) |
| **P0-N3-L1** | `deactivate()` during Finish's writes: `cancelSession('deactivate')` returns at once on `finishing`, so the scope is disposed under `add -u` / amend / `--continue`. Await `take(sessionStatus, s => s !== 'finishing' \|\| throwAbort())` for `reason === 'deactivate'` inside the 4 s budget | **L1** (third re-review) | `src/model/session.ts` `cancelSession`, `src/index.ts` | T8 reload drill | Open (non-gating) |
| **P0-N3-L2** | Cancel discards Edit-here edits (`--abort`) with no hint when `editedPaths.length > 0`; design-system §6 proposes "Abort may discard edits made during this rebase" | **L2** (third re-review) | `src/model/sidebar.ts` | design pass | Open (non-gating) |
| **P0-N3-L3** | A cancelled Start emits no "Review cancelled." — only git's "Applied autostash." or nothing | **L3** (third re-review) | `src/model/session.ts` `startFailed` | UX | Open (non-gating) |

F1–F9 landed 2026-09-11. **Re-review 2026-09-11:** F1–F5, F7–F9 confirmed; F6 reopened, F10 / F11 filed. **Implementer re-review pass 2026-09-11:** F6, F10, F11 closed in code (T9 / T10 / m8–m10). **Second re-review 2026-09-11:** F6, F11 confirmed (F6 at five Cancel offsets + the post-`finally` path); **F10 reopened**, **F12 / F13 filed**. **Implementer second re-review pass 2026-09-11:** F10, F12, F13 closed in code (T11 / T12 / refuse-in-progress / Cancel gated off during `finishing`). **Third re-review 2026-09-11: approved** — F10, F12, F13 confirmed by execution (P6 slow-abort, P1 cancel + deactivate, P3 foreign rebase); L1–L3 filed as non-gating Lows. **T8 still open:** §10.6 drills (add reload mid-Finish per L1) and the Phase 13 / product v0.3 boxes are unrun.
